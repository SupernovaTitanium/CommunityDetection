...
function  [c, Z,W] = LFlasso_over(R0_sp,R1_sp,lambda,Z0,W0,T,threshold)
...
for t = 1:T
	...
	if length(c)==0
		A = setA_dense(zeros(n,1),zeros(1,n),R1_sp);
		z = MixMaxCut(A,SDP_rank,SDP_iter);
	else
		Zc = Z * diag(sqrt(c));
		[W_o,~] = w_solver(R0_sp,R1_sp,Zc,Zc',zeros(size(Z,2),n),lambda);
		A = setA_dense(Zc,W_o,R1_sp);
		z = MixMaxCut(A,SDP_rank,SDP_iter);
	end
	...
	if mod(t,5)==0
			...
			[acc,rec,prec,F1,F1_2,auc] = all_statistic(full(R1_sp),ones(size(R1_sp,1),size(R1_sp,2)),R_guess,Z,1,threshold);
			[~,~,~,~,~,g_auc] = all_statistic(full(R1_sp),ones(size(R1_sp,1),size(R1_sp,2)),Rt_guess,Z,1,threshold);
