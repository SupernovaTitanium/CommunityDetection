...
void lbfgs_mex_interface_denseZ(sp_matrix R0,sp_matrix R1,double* Z,double* Zt,double* W0,double lambda,double* W_opt,double* value,int N,int K){
    int i, ret = 0;
    lbfgsfloatval_t fx;
    lbfgsfloatval_t *x = lbfgs_malloc(N*K);
    lbfgs_parameter_t param;
    sp_matrix_Z* RZ = (sp_matrix_Z*)malloc(sizeof(sp_matrix_Z)*1) ;
    if (x == NULL) {
        printf("ERROR: Failed to allocate a memory block for variables.\n");
        return;
    }
    ...
    param.epsilon = 1e-2;
    //printf("start lbfgs\n");
    ret = lbfgs(N*K, x, &fx, evaluate_dense_p, progress, RZ, &param);
    /* Report the result. */
    // printf("L-BFGS optimization terminated with status code = %d\n", ret);    
    for(int i=0;i<N*K;i++){
      W_opt[i] = x[i];
...
void usage()
{
	mexErrMsgTxt("Usage:[sparse] function [W_o,value] = w_solver(R0_sp',R1_sp',Z'_sparse,W0,lambda)\nUsage:[dense]  function [W_o,value] = w_solver(R0_sp,R1_sp,Z,W0,lambda,Z");
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]){
  ...
  if (!(mxIsSparse(prhs[2]))){
        ...
        Zt = mxGetPr(prhs[3]);
        lbfgs_mex_interface_denseZ(R0_sp,R1_sp,Z,Zt,W0,lambda,mxGetPr(plhs[0]),mxGetPr(plhs[1]),N,K);
  }else{  
